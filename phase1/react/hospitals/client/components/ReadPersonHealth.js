  ...
  useEffect(() => {
    if (dataReceive === 'loading') {
      const callRest = async () => {
        const response = await axios.get('/api/cadanacv1/actors/' + profileState.context + '/patient/' + dataSend.pkPersonHealthID + '/' + dataSend.virusType);
...
export default ReadPersonHealth;
